...
export default function more() {
  ...
  return (
    <ScrollView contentContainerClassName='px-6 pt-6 pb-32 gap-7'>
      ...
      <View>
        ...
        <View className={`gap-3 px-5 overflow-hidden ${isAccountDropdownOpen ? '' : 'h-0'}`}>
          {moreLinks.accountSettingsLinks.map((link, index) => (
            <View 
              key={index} 
              className={`bg-white rounded-lg p-4 ${index ===  0 ? 'mt-3' : ''}`} 
              onTouchStart={() => router.push({ 
                pathname: '/subpages/[id]',
                params: { id: index + 1, title: link.text }
              })} 
            >
